...
namespace FFRPG.Windows;

public class MainWindow : Window, IDisposable
{
    private readonly FFRPG plugin;
    public static Configuration Config { get; set; }
    public Game Game;
    public PlayerList PlayerList;
    private MainTab currentMainTab = MainTab.PlayerList;

    public MainWindow(FFRPG plugin) : base(
    ...
    {
        if (ImGui.BeginTabBar("FFRPGMainTabBar", ImGuiTabBarFlags.NoTooltip))
        {
            if (ImGui.BeginTabItem("Player List###FFRPG_PlayerList_MainTab"))
            {
                currentMainTab = MainTab.PlayerList;
                ImGui.EndTabItem();
            }

            if (ImGui.BeginTabItem("Game###FFRPG_Match_MainTab"))
            {
                currentMainTab = MainTab.Game;
                ImGui.EndTabItem();
            }

            if (ImGui.BeginTabItem("About###FFRPG_About_MainTab"))
            {
